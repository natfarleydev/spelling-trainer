...
| Written sentences for the spelling-pattern words that Tatoeba does not cover well | Our own work | CC0 1.0 | Not applicable | [src/sentences/bank/writtenPattern.ts](src/sentences/bank/writtenPattern.ts), [src/sentences/bank/writtenPatternExtra.ts](src/sentences/bank/writtenPatternExtra.ts) |
| Written sentences for the CYP-LEX book words that Tatoeba does not cover well | Our own work | CC0 1.0 | Not applicable | [src/sentences/bank/writtenCyplex.ts](src/sentences/bank/writtenCyplex.ts) |
...
| The words that children aged 7 to 9 meet in books, for the word list of the mining tool (**test data and mining input only**, not in the app) | [CYP-LEX](https://osf.io/tnu8k/), file `main_cyplex79.csv`, in the project [Children and Young People's Books Lexicon](https://osf.io/squ49/), downloaded 2026-09-16 | [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) | We kept only the word forms that pass a filter for frequency, book spread and word type, and we removed the rest of the data. Claude then removed the words that are not correct for a spelling test for children. | [src/sentences/testing/cyplexWords.ts](src/sentences/testing/cyplexWords.ts) |
...
Citation for CYP-LEX: Korochkina, M., Marelli, M., Brysbaert, M. and Rastle, K. (2024). The Children and Young People's Books Lexicon (CYP-LEX): A large-scale lexical database of books read by children and young people in the United Kingdom. Quarterly Journal of Experimental Psychology. https://doi.org/10.1177/17470218241229694
